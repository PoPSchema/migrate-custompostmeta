<?php
namespace PoP\PostMeta;

class Utils
{
    public static function getMetaKey($meta_key)
    {
        $functionapi = FunctionAPIFactory::getInstance();
        return $functionapi->getMetaKey(\PoP\Meta\Utils::getMetakeyPrefix().$meta_key);
    }

    public static function getPostMeta($post_id, $key, $single = false)
    {
        $functionapi = FunctionAPIFactory::getInstance();
        return $functionapi->getPostMeta($post_id, self::getMetaKey($key), $single);
    }
    public static function updatePostMeta($post_id, $key, $values, $single = false, $boolean = false)
    {
        $values = \PoP\Meta\Utils::normalizeValues($values);

        // Add the values as independent values so each one of them can be searched using EXISTS on WP_Query
        $functionapi = FunctionAPIFactory::getInstance();
        $functionapi->deletePostMeta($post_id, self::getMetaKey($key));
        foreach ($values as $value) {
            // If dealing with boolean values, do not save a "false" in the DB, so we can use "EXISTS" to find all the entries with "true"
            if ($boolean && !$value) {
                continue;
            }
            $functionapi->addPostMeta($post_id, self::getMetaKey($key), $value, $single);
        }
    }
    public static function addPostMeta($post_id, $key, $value, $unique = false)
    {
        $functionapi = FunctionAPIFactory::getInstance();
        $functionapi->addPostMeta($post_id, self::getMetaKey($key), $value, $unique);
    }
    public static function deletePostMeta($post_id, $key, $value = '')
    {
        $functionapi = FunctionAPIFactory::getInstance();
        $functionapi->deletePostMeta($post_id, self::getMetaKey($key), $value);
    }
}
